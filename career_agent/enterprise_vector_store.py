"""Multilingual local Qdrant index for the enterprise-official augmented corpus."""
...
from .enterprise_corpus import load_enterprise_job_records
from .large_vector_store import embedding_text
from .multilingual_all_job_vector_store import MODEL_NAME


COLLECTION_NAME = "career_agent_enterprise_official_all_jobs_v1"
DEFAULT_STORAGE = Path(__file__).resolve().parents[1] / "qdrant_storage" / "career_agent_enterprise_official_all_jobs_v1"
...
class EnterpriseJobVectorStore:
    ...
    def rebuild(self, *, embedding_batch_size: int = 64, upsert_batch_size: int = 256) -> dict[str, Any]:
        ...
        if not records:
            raise ValueError("No enterprise-augmented JD records found")
