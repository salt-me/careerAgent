"""API for semantic search over 14k+ JD records with direct company ATS URLs."""
...
from .enterprise_search import EnterpriseJobSearchStore
...
app = FastAPI(title="CareerAgent Enterprise Official JD Service", version="1.3.0")
store = EnterpriseJobSearchStore()
...
def report() -> dict:
    records, deduplication = load_enterprise_job_records()
    result = enterprise_report(records)
