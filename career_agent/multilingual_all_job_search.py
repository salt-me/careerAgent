"""Multilingual semantic retrieval over the Qdrant all-job collection."""
...
from .multilingual_all_job_vector_store import COLLECTION_NAME, DEFAULT_STORAGE, MODEL_NAME


@dataclass(frozen=True)
class MultilingualSearchHit:
    id: str
    title: str
    company: str
    position: str
...
class MultilingualAllJobSearchStore:
    ...
    def search(
    ...
    ) -> list[MultilingualSearchHit]:
        if not self.is_ready():
            raise RuntimeError("Multilingual all-job vector store is not indexed. Run MultilingualAllJobVectorStore().rebuild().")
        ...
        points = self.client.query_points(
            COLLECTION_NAME, query=vector, query_filter=query_filter, limit=limit, with_payload=True
        ).points
        return [
            MultilingualSearchHit(
