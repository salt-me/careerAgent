"""Multilingual Qdrant index for the canonical production JD corpus."""
...
COLLECTION_NAME = "career_agent_production_all_jobs_v1"
DEFAULT_STORAGE = Path(__file__).resolve().parents[1] / "qdrant_storage" / "career_agent_production_all_jobs_v1"
...
class ProductionJobVectorStore:
    ...
    def rebuild(self, *, embedding_batch_size: int = 64, upsert_batch_size: int = 256) -> dict[str, Any]:
        ...
        if not records:
            raise ValueError("No production JD records found")
