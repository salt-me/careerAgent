"""Retrieval path for the canonical 10k all-job vector collection."""
...
from .verified_all_job_vector_store import COLLECTION_NAME, DEFAULT_STORAGE


@dataclass(frozen=True)
class VerifiedSearchHit:
    id: str
    title: str
    company: str
    position: str
...
class VerifiedAllJobSearchStore:
    ...
    ) -> list[VerifiedSearchHit]:
        if not self.is_ready():
            raise RuntimeError("Verified all-job vector store is not indexed. Run VerifiedAllJobVectorStore().rebuild().")
