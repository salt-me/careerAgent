"""Qdrant + BGE batch index for the verified 10k all-job JD corpus."""
...
from .large_corpus import load_large_job_records
from .vector_retrieval import MODEL_NAME


COLLECTION_NAME = "career_agent_all_jobs_10k_v1"
DEFAULT_STORAGE = Path(__file__).resolve().parents[1] / "qdrant_storage" / "career_agent_all_jobs_10k_v1"
...
def embedding_text(record: dict[str, Any]) -> str:
    """Make title/function searchable while limiting each embedding input to 6k chars."""
    return "\n".join(
        (
            f"Title: {record['title']}",
            f"Function: {record['position']}",
...
class LargeJobVectorStore:
    ...
    def rebuild(self, *, embedding_batch_size: int = 64, upsert_batch_size: int = 256) -> dict[str, Any]:
        ...
        if not records:
            raise ValueError("No verified records found")
